...
/**********************************************************************************************************/		
		...
		public function geraNumeros(){  				
			if ($this->ida > 1) { 
                           echo "<a class=\"btn btn-danger\" href=\"$this->fileName&pg=$this->pagAnt\" title=\"$this->pagAnt\">Anterior</a>\n";
					   }
					   ...
					   while(($lnk_impressos < $this->numeroPaginas) and ($lnk_impressos < $this->maxLink)){ 
				           $lnk_impressos ++; 
							  ...
							  if ($this->pagAtual == $lnk_impressos){
								 echo "<span class=\"btn btn-danger\">$lnk_impressos</span> \n";
				           //mostra os números das 
						   }else{
                              echo "<a  class=\"btn btn-danger\" href=\"$this->fileName&pg=$lnk_impressos\" title=\"$lnk_impressos\">$lnk_impressos</a>\n";
						      } 
					       }
                      // mostra o link PRÓXIMO >>
					  if ($this->registroFinal < $this->resultadoTotal){
                            echo"<a class=\"btn btn-danger\"href=\"$this->fileName&pg=$this->proxPag\" title=\"$this->proxPag\">Pr&oacute;ximo</a>\n";
					 }				
		}
